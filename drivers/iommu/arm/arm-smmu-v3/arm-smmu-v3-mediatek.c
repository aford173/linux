...
static bool smmuwp_process_tf(struct arm_smmu_device *smmu)
{
	...
	for (i = 0; i < mtk_smmu_v3->plat_data->tbu_cnt; i++) {
		...
		dev_err(smmu->dev, "TF read TBU_id-%d fault_id:0x%x(0x%x) iova:0x%llx sid:%d ssid:%d ssidv:%d\n",
			i, fault_id, axiid, fault_iova, sid, ssid, ssidv);
...
write:
		...
		dev_err(smmu->dev, "TF write TBU_id-%d fault_id:0x%x(0x%x) iova:0x%llx sid:%d ssid:%d ssidv:%d\n",
			i, fault_id, axiid, fault_iova, sid, ssid, ssidv);
	}
...
static const struct arm_smmu_impl_ops mtk_smmu_v3_impl_ops = {
...
struct arm_smmu_device *arm_smmu_v3_impl_mtk_init(struct arm_smmu_device *smmu)
{
	...
	plat_data = mtk_smmu_v3_get_plat_data(dev->of_node);
	if (!plat_data)
	...
	mtk_smmu_v3->smmu.impl_ops = &mtk_smmu_v3_impl_ops;
	mtk_smmu_v3->plat_data = plat_data;

	if (MTK_SMMU_HAS_FLAG(mtk_smmu_v3->plat_data, SMMU_REQUIRE_PARENT)) {
		parent_node = of_parse_phandle(dev->of_node, "power-domains", 0);
		if (!parent_node) {
			dev_err(dev, "Missing power-domains property\n");
		...
		of_node_put(parent_node);
		if (!parent_pdev) {
			dev_err(dev, "Power domain device not found\n");
			return ERR_PTR(-ENODEV);
		}

		if (!platform_get_drvdata(parent_pdev)) {
			dev_err(dev, "Power domain driver not ready, deferring\n");
	...
	smmu_init_wpcfg(&mtk_smmu_v3->smmu);
